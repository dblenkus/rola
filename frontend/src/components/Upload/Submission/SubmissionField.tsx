...
class SubmissionField extends React.Component<SubmissionFieldProps> {
  render(): React.ReactNode {
    const { submission, handleSubmissionChange, handleImageChange } =
      this.props;

    const onSubmissionChange = (payload: InputChange): void =>
      handleSubmissionChange(payload);

    const onImageChange =
      (image_id: number) =>
      (payload: { file: File | undefined }): void =>
        handleImageChange(image_id, payload);

    const titleField = (
      <TitleField submission={submission} onChange={onSubmissionChange} />
    );
